...
import glob, json
from trinity_dataloader import SD15TrainDataset
import sys
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                batch["pixel_values"] = transform_img(batch["pixel_values"], args)
                ...
                down_block_res_samples, mid_block_res_sample = controlnet(
                    ...
                    controlnet_cond=controlnet_image.squeeze(),
                    return_dict=False,
                )
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        save_path = os.path.join(args.output_dir, f"controlnn-checkpoint-{global_step}")
