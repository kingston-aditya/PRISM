...
from trinity_dataloader import CntTrainDataset
...
def main(args):
    ...
    def collate_fn(batch):
        original_pixel_values = torch.stack([example["orig_imgs"] for example in batch])
        original_pixel_values = original_pixel_values.to(memory_format=torch.contiguous_format).float()
        edited_pixel_values = torch.stack([example["edit_imgs"] for example in batch])
        edited_pixel_values = edited_pixel_values.to(memory_format=torch.contiguous_format).float()
        input_ids = torch.stack([example["prompt_embeds"] for example in batch])
        filenames = [example["filenames"] for example in batch]
        return {
    ...
    precomputed_dataset = CntTrainDataset(json_obj, args, tokenizer)
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            dirs = [d for d in dirs if d.startswith("pix2pix-checkpoint")]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                latents = vae.encode(batch["edited_pixel_values"].squeeze(1).to(weight_dtype)).latent_dist.sample()
                ...
                original_image_embeds = vae.encode(batch["original_pixel_values"].squeeze(1).to(weight_dtype)).latent_dist.mode()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        save_path = os.path.join(args.output_dir, f"pix2pix-checkpoint-{global_step}")
