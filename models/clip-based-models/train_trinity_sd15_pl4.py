...
def main(args):
    ...
    params_to_optimize = list(lora_layers) + list(proj_layer.parameters())
    ...
    unet, optimizer, train_dataloader, lr_scheduler, proj_layer = accelerator.prepare(
        unet, optimizer, train_dataloader, lr_scheduler, proj_layer
    )
    ...
    if args.resume_from_checkpoint == "latest":
        all_pths = glob.glob(os.path.join(args.output_dir, "sd15-pl4-checkpoint-*"))
        if len(all_pths) != 0:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(proj_layer):
                ...
                if accelerator.sync_gradients:
                    params_to_clip = list(lora_layers) + list(proj_layer.parameters())
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        save_path = os.path.join(args.output_dir, f"sd15-pl4-checkpoint-{global_step}")
