...
from utilities import run_mllm
import pdb 
...
def run_init_real():
    ...
    for batch in tqdm(dtel, desc="Saving data"):
        temp = {"filname":{}, "imgs":{}}; k1 = 0
        for img in batch:
            img.save(os.path.join(args["output_img_folder"], str(k)+".png"))
            temp["filname"][k1] = os.path.join(args["output_img_folder"], str(k)+".png")
            temp["imgs"][k1] = img
            k1 += 1
