...
def run_final_syn():
    ...
    for prt in tqdm(caps, desc="Processing"):
        ...
        a = {}; b={}
        for i in imgs:
            i.save(os.path.join(args["output_img_folder"], str(k)+".png"))
            a[k] = os.path.join(args["output_img_folder"], str(k)+".png")
            b[k] = i
            k+=1
    ...
    f = open(os.path.join(args["output_metadata_folder"], "metadata.jsonl"), "w")
