...
from diffusers.models.transformers.transformer_sd3 import QwenVLSD3Transformer2DModel, SD3Transformer2DModel, QwenVLSD3_DirectMap_Transformer2DModel, LlamaRMSNorm 
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--do_validation", action="store_true", help="Do validation or not.")
...
def main(args):
    ...
    if args.gradient_checkpointing:
        ...
        connector.enable_gradient_checkpointing()
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for i, model in enumerate(models):
                ...
                if hasattr(unwrapped, "connector"):
    ...
    def load_model_hook(models, input_dir):
        for _ in range(len(models)):
            ...
            elif hasattr(unwrap_model(model), "connector"):
                model.connector.save_pretrained(
                    os.path.join(input_dir, "connector.pt")
                )
            else:
    ...
    if accelerator.is_main_process:
        tracker_name = "dreambooth-sd3-connector"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(connector):
                # prepare the inputs.
                ...
                # process the image inputs for LMM.
                ...
                ## Siglip inputs
                siglip_inputs = {k: v.to(model_input.device, dtype=weight_dtype) for k, v in siglip_inputs.items()}
                for k, v in siglip_inputs.items():
                    if torch.is_floating_point(v):
                        siglip_inputs[k] = v.half()
                if 'attention_mask' not in siglip_inputs:
                    pad_token_id = siglip_processor.tokenizer.pad_token_id or 1
                    siglip_inputs['attention_mask'] = (siglip_inputs['input_ids'] != pad_token_id).long()

                inputs['siglip_inputs'] = siglip_inputs
                ...
                if args.keep_context_embedder:
                    ...
                    model_pred, model_pred_conn, conn_hidden_state, enc_hidden_state = transformer(
                        **inputs
                    )[0]

                loss1 = torch.mean((enc_hidden_state.float() - conn_hidden_state.float())**2, 1)
                loss2 = torch.mean((model_pred.float() - model_pred_conn.float())**2, 1)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0 and args.do_validation:
