...
class TrinityPipeline(
    ...
    def encode_object(self, batch, img_encoder_1, img_encoder_2, img_tokenizer_1, img_tokenizer_2, device):
        object_embeds_list = []

        img_encoder_1 = img_encoder_1.to(device)
        img_encoder_2 = img_encoder_2.to(device)

        img_tokenizers = [img_tokenizer_1, img_tokenizer_2]
        img_encoders = [img_encoder_1, img_encoder_2]
        ...
        tok_sz = prompt_embeds.shape[-2]//len(batch)
        if len(batch)==1:
            prompt_embeds = torch.cat((prompt_embeds, prompt_embeds[0,:tok_sz,:].unsqueeze(0), prompt_embeds[0,:tok_sz,:].unsqueeze(0)), dim=-2)
        elif len(batch) == 2:
    ...
    def multimodal_encode_prompt(self, prompt_embeds, object_prompt_embeds, trinity, proj_layer, device):
        ...
        cat_prompt_embeds = cat_prompt_embeds/torch.norm(cat_prompt_embeds, p=2, dim=-1, keepdim=True)
        
        # pass it through trinity
        trinity_embeds = trinity(cat_prompt_embeds, cat_prompt_embeds, txt_tok_len, img_tok_len, typ="causal")
        trinity_embeds = trinity_embeds/torch.norm(trinity_embeds, p=2, dim=-1, keepdim=True)
        return trinity_embeds
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 3 - Get the prompt embeddings
        # 3a. Encode input prompt
        lora_scale = (
        ...
        object_prompt_embeds = self.encode_object(object_imgs, img_encoder_one, img_encoder_two, img_tokenizer_one, img_tokenizer_two, device)

        # 3c. Concatenate objects and prompts
        # load trinity and projection layer
        all_pths = glob.glob(os.path.join(args["input_dir"], "sdxl-checkpoint-*"))
        if len(all_pths) == 0:
            print("WEIGHTs CANNOT BE found !!! Exiting")
        path_name = sorted(all_pths, key=lambda x: int(x.split('-')[-1].split('.')[0]))[-1]
        trinity.load_state_dict(torch.load(os.path.join(args["input_dir"], path_name, "trinity_checkpoint"+".pt"), weights_only=True))
        proj_model.load_state_dict(torch.load(os.path.join(args["input_dir"], path_name, "proj_checkpoint"+".pt"), weights_only=True))
        prompt_embeds = self.multimodal_encode_prompt(prompt_embeds, object_prompt_embeds, trinity, proj_model, device)
