...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output, img_feat, txt_feat, cls = self.model_inference(input)
            a.append(img_feat)
            b.append(txt_feat[int(label[0].cpu().detach().numpy())])
            c.append(output.cpu().detach().numpy())
            self.evaluator.process(output, label)

        a = np.asarray(a).reshape(-1,512)
        b = np.asarray(b).reshape(-1,512)
        c = np.asarray(c).reshape(-1,102)
        np.save("/data/aditya/clip_image_feat.npy",a)
        np.save("/data/aditya/clip_text_feat.npy",b)
        np.save("/data/aditya/clip_logits_feat.npy",c)

        results, pred, gt = self.evaluator.evaluate()
        np.save("/data/aditya/clip_gt_feat.npy",gt)
        np.save("/data/aditya/clip_pred_feat.npy",pred)
